local capi = { client = client, mouse = mouse, screen = screen, keygrabber = keygrabber }
...
local function init()
	...
	local padding = beautiful.collision_focus_padding or 4
	local bg = beautiful.collision_focus_bg or beautiful.bg_alternate or "#ff00F0"
	local fg = beautiful.collision_focus_fg or beautiful.fg_normal or "#0000ff"
	local bg_focus = beautiful.collision_focus_bg_center or beautiful.bg_urgent or "#ff00ff"
	local sw = beautiful.collision_shape_width or 25
	local sh = beautiful.collision_shape_height or 25
			...
			v ~= "center" and {
				{
					...
					shape = shape.transform(col_utils.arrow_path2):rotate_at(x, y, (k - 1) * (2 * math.pi) / 4),
				},
				margins = padding,
...
local function float_move_max(dir, c)
	return ({
		left = { x = capi.screen[c.screen].workarea.x },
		right = { x = capi.screen[c.screen].workarea.width + capi.screen[c.screen].workarea.x - c:geometry().width },
		up = { y = capi.screen[c.screen].workarea.y },
		down = { y = capi.screen[c.screen].workarea.y + capi.screen[c.screen].workarea.height - c:geometry().height },
	})[dir]
end
...
local function floating_clients()
	local ret = {}
	for v in
		util.table.iterate(client.visible(), function(c)
			return c.floating
		end)
	do
		ret[#ret + 1] = v
